...
const sampleUsers = [
	...
	},
	{
		...
		email: "aleksandar.petrov@example.com",
		password: "password123",
		phone: "+389 76 567 890",
		location: "Gostivar, North Macedonia",
	},
	{
		name: "Tamara Nikolovska",
		email: "tamara.nikolovska@example.com",
		password: "password123",
		phone: "+389 77 678 901",
		location: "Strumica, North Macedonia",
	},
	{
		name: "Marko Jovanovski",
		email: "marko.jovanovski@example.com",
		password: "password123",
		phone: "+389 78 789 012",
		location: "Kochani, North Macedonia",
	},
	{
		name: "Jovana Spasovska",
		email: "jovana.spasovska@example.com",
		password: "password123",
		phone: "+389 79 890 123",
...
async function seedDummyData() {
	try {
		...
		const existingDummyUsers = await User.find({
			email: { $in: sampleUsers.map((user) => user.email) },
		});

		if (existingDummyUsers.length > 0) {
			console.log("✅ Dummy users already exist, skipping user creation");
			console.log(`Found ${existingDummyUsers.length} existing dummy users`);
		} else {
		...
		// Check if dummy products already exist (check by specific titles)
		const dummyProductTitles = sampleProducts.map((p) => p.title);
		const existingDummyProducts = await Product.find({
			title: { $in: dummyProductTitles },
		});

		if (existingDummyProducts.length > 0) {
			console.log("✅ Dummy products already exist, skipping product creation");
			console.log(
				`Found ${existingDummyProducts.length} existing dummy products`
			);
		} else {
			console.log("📦 Creating dummy products...");

			// Get existing users to assign as sellers for dummy products
			const allUsers = await User.find();
			if (allUsers.length === 0) {
				console.error(
					"❌ No users found to assign as sellers for dummy products"
				);
				return;
			}
			...
			for (const productData of sampleProducts) {
				...
				console.log(`Created dummy product: ${product.title}`);
			}
